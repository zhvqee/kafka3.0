...
class KafkaApis(val requestChannel: RequestChannel,
  ...
  override def handle(request: RequestChannel.Request, requestLocal: RequestLocal): Unit = {
    try {
      ...
      request.header.apiKey match {
        case ApiKeys.PRODUCE => handleProduceRequest(request, requestLocal) //处理 消息的存储
  ...
  def handleProduceRequest(request: RequestChannel.Request, requestLocal: RequestLocal): Unit = {
    ...
    val authorizedRequestInfo = mutable.Map[TopicPartition, MemoryRecords]() //用于存储 可以的记录的log 的map
    ...
    produceRequest.data.topicData.forEach(topic => topic.partitionData.forEach { partition =>
      ...
      if (!authorizedTopics.contains(topicPartition.topic))
        ...
        try {
          ...
          authorizedRequestInfo += (topicPartition -> memoryRecords) //日志
        } catch {
          case e: ApiException =>
    ...
    if (authorizedRequestInfo.isEmpty)
      ...
      replicaManager.appendRecords(
        ...
        entriesPerPartition = authorizedRequestInfo, // 请求的数据
